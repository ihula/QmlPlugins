...
## 更新摘要
**变更内容**
- 完全移除了BaseInfoSender机制，替换为新的BaseMsgSender架构
- 信息中心重命名为消息中心，API和接口发生重大变化
- 更新了消息发送机制，从BaseInfoSender改为BaseMsgSender
- 修改了消息处理流程和订阅管理方式
- 更新了QML界面的交互方式和数据绑定
...
**更新** 本版本完全移除了原有的BaseInfoSender机制，采用了全新的BaseMsgSender架构，实现了更简洁高效的组件间通信模式。
...
- BaseMsgSender：新的消息发送基类，统一发出messageEmitted信号，供MessageCenter订阅。
- DbManager：继承BaseMsgSender的典型发送方，负责数据库操作并在错误时上报消息。
- Common：定义消息结构MessageInfo、错误码枚举StatusCode、提示类型PromptType等。
- Configer：提供用户上下文（用户名、账号），用于消息持久化时附加用户信息。
- QML界面：MessageCenterForm.qml展示历史错误，支持日期筛选、删除操作等功能。

**更新** 核心组件已完全重构，BaseInfoSender被BaseMsgSender替代，提供了更简洁的接口设计。
...
**更新** 架构已完全重构，移除了BaseInfoSender机制，采用更简洁的BaseMsgSender模式。
...
alt "错误码非成功"
Center->>FS : "appendData(QVariantMap)"
end
...
**图表来源**
- [messagecenter.cpp:99-115](file://src/messagecenter.cpp#L99-L115)
- [basemsgsender.h:27-35](file://src/basemsgsender.h#L27-L35)
- [dbmanager.cpp:118-224](file://src/dbmanager.cpp#L118-L224)
...
**更新** API已更新，使用QVariantMap替代QJsonArray，提供了更灵活的数据处理方式。
...
+getDatas(date) QList~QVariantMap~
+deleteData(id) int
+deleteAllData() int
+hasNewInfo() bool
...
**图表来源**
- [messagecenter.h:26-123](file://src/messagecenter.h#L26-L123)
- [messagecenter.cpp:14-115](file://src/messagecenter.cpp#L14-L115)
- [basemsgsender.h:10-38](file://src/basemsgsender.h#L10-L38)
- [configer.h:98-277](file://src/configer.h#L98-L277)

**章节来源**
- [messagecenter.h:35-123](file://src/messagecenter.h#L35-L123)
- [messagecenter.cpp:14-115](file://src/messagecenter.cpp#L14-L115)
...
**更新** BaseMsgSender提供了更简洁的接口，移除了BaseInfoSender的复杂机制，简化了消息发送流程。
...
**更新** DbManager现在直接继承BaseMsgSender，简化了消息发送流程，移除了BaseInfoSender的间接机制。
...
**章节来源**
- [dbmanager.h:52-216](file://src/dbmanager.h#L52-L216)
- [dbmanager.cpp:14-974](file://src/dbmanager.cpp#L14-L974)

### 消息类型与负载格式
- 消息结构：MessageInfo包含文本、状态码、提示类型三要素。
- 状态码：StatusCode枚举按模块划分区间，便于分类统计与定位。
- 提示类型：PromptType区分Toast、Log、Confirmation、Error四类，决定UI呈现方式。
- 序列化：消息通过Qt元对象系统传递，无需额外序列化；持久化时使用QVariantMap/QSettings INI格式。

**更新** 消息结构已更新为MessageInfo，使用StatusCode替代ErrorCode，提供了更清晰的状态管理。
...
- [common.h:26-33](file://src/common.h#L26-L33)
...
### 生命周期与订阅管理
- 单例生命周期：通过SINGLETON宏提供线程安全的单例实例，支持销毁与状态查询。
...
**更新** 订阅管理已简化，BaseMsgSender自动处理订阅和解绑，减少了手动管理的工作量。
...
- [messagecenter.h:88-103](file://src/messagecenter.h#L88-L103)
...
- 发送消息（业务模块继承BaseMsgSender并发出信号）
  - 参考：[basemsgsender.h:27-35](file://src/basemsgsender.h#L27-L35)
- 订阅消息（由发送方自动完成，也可手动调用connectRecv/disconnectRecv）
  - 参考：[messagecenter.h:88-103](file://src/messagecenter.h#L88-L103)
...
**更新** 依赖关系已简化，移除了BaseInfoSender的间接依赖，提高了模块间的清晰度。
...
**更新** 性能优化保持不变，BaseMsgSender的简化设计进一步提升了消息处理效率。

**章节来源**
- [dbmanager.cpp:51-117](file://src/dbmanager.cpp#L51-L117)
- [messagecenter.cpp:26-115](file://src/messagecenter.cpp#L26-L115)

## 故障排查指南
- 数据库连接失败
  - 现象：connect返回错误码，lastError被设置。
  - 排查：确认数据库文件路径、权限；检查WAL模式与busy_timeout设置。
  - 参考：[dbmanager.cpp:51-91](file://src/dbmanager.cpp#L51-L91)
- 数据库操作失败
  - 现象：execSql/updateDatas/appendDatas返回错误码，lastError被设置。
  - 排查：检查SQL语法、表结构、参数绑定；确认事务守卫是否正确提交/回滚。
  - 参考：[dbmanager.cpp:597-618](file://src/dbmanager.cpp#L597-L618)
- 消息未到达UI
...
- 错误信息未持久化
  ...
  - 参考：[messagecenter.cpp:99-115](file://src/messagecenter.cpp#L99-L115)

**更新** 故障排查指南已更新，反映了新的BaseMsgSender架构和API变化。

**章节来源**
- [dbmanager.cpp:51-91](file://src/dbmanager.cpp#L51-L91)
- [dbmanager.cpp:597-618](file://src/dbmanager.cpp#L597-L618)
- [basemsgsender.cpp:8-10](file://src/basemsgsender.cpp#L8-L10)
- [MessageCenterForm.qml:215-231](file://src/QmlUI/MessageCenterForm.qml#L215-L231)
- [messagecenter.cpp:99-115](file://src/messagecenter.cpp#L99-L115)

## 结论
消息中心模块通过BaseMsgSender统一消息出口、MessageCenter集中处理与持久化、DbManager等发送方提供可靠数据通道，形成清晰的发布-订阅架构。配合Singleton单例、线程本地错误缓存与互斥保护，实现了高内聚、低耦合、可扩展的消息体系。新的BaseMsgSender架构移除了复杂的BaseInfoSender机制，提供了更简洁高效的组件间通信模式。建议在后续迭代中引入消息优先级与队列策略，增强大规模并发下的吞吐与稳定性。

**更新** 结论已更新，反映了BaseMsgSender架构的优势和简化效果。

## 附录

### 消息可靠性与恢复策略
- 可靠性保障
  - 状态码过滤：仅非成功状态码写入持久化，避免噪声干扰。
  - 异步通知：信号槽异步传播，降低阻塞风险。
...
**更新** 恢复策略保持不变，BaseMsgSender架构提供了更好的消息可靠性保障。

**章节来源**
- [messagecenter.cpp:99-115](file://src/messagecenter.cpp#L99-L115)
- [dbmanager.cpp:119-224](file://src/dbmanager.cpp#L119-L224)
- [dbmanager.cpp:597-618](file://src/dbmanager.cpp#L597-L618)